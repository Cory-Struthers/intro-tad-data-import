...
`readtext` allows you to load multiple documents at the same time, each creating a different row in a data frame. As shown below, the package provides options to convert text in the file names to document variables. In the example, we use the state and dates in the names -- separated by a space -- to create two document variables (state, date).
...
**Question 1. Review some of the text in the "transcripts_all" data frame. What do you notice about the text records? How might the metadata mischaracterize the text records?**
...
**Question 2 (BREAKOUT). Create a data frame where each document (including text) is a row using the sample of legislative bills in the "bills" folder. Upload and attach the metadata included in that folder("bills_leg_attributes.xls"). Save your data frame csv file.**
